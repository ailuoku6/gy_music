        ...
        return (
            <View style={styles.container}>
                <TextInput
                    ...
                    onChangeText={text => this.setState({ SingerName:text })}
                />
                <TextInput
                    label='歌手性别'
                    value={this.state.SingerSex}
                    onChangeText={text => this.setState({ SingerSex:text })}
                />
                <View>
                    <TextInput
                        label='歌手头像'
                        value={this.state.singerAvatar}
                        onChangeText={text => this.setState({ singerAvatar:text })}
                    />
                    <Button onPress={()=>{
                        this.setState({
                            singerAvatar:randomImg()
                        })
                    }} mode={'contained'}>随机生成头像</Button>
                </View>

                <TextInput
                    label='歌手简介'
                    value={this.state.singerIntro}
                    onChangeText={text => this.setState({ singerIntro:text })}
                />
                <Button onPress={()=>{
                    ...
                    //     return;
                    // }
                    if (!judgeValue(this.state.SingerName,this.state.SingerSex,this.state.singerAvatar,this.state.singerIntro)){
                        ToastAndroid.show("请将信息填写完整",ToastAndroid.SHORT)
                        return;
                    }
                    const DataBaseModule = NativeModules.DataBaseModule;
                    DataBaseModule.AddSinger(this.state.SingerName,this.state.SingerSex,this.state.singerAvatar,this.state.singerIntro).then((result)=>{
                        if (result==='succ'){
...
export default connect(mapStateToProps)(AddSinger);
